...
|Commerce |Recommendation |Yambda-5B |v6.1
...
|Commerce |Recommendation |dlrm_v4_hstu |0.75 AUC |v6.1
|===
...
In the case of dlrm_v4_hstu benchmark, the training data is not shuffled. The dataset is carved by wall clock into a Global Temporal Split, and within the training portion the timeline is sliced into fixed-duration windows (`streaming_window_seconds = 86400`) which are traversed in strict forward chronological order: the loop trains window `T`, then evaluates window `T+1`, then advances. Reordering windows, or any shuffling that moves samples across window boundaries, is prohibited, since doing so would leak future interactions into the training history. Within a window, anchors are consumed in dataset order; the reference exposes an optional partial intra-window shuffle (`STREAMING_SHUFFLE_FRACTION`) which must be left at its default of `0.0` for closed division submissions.
...
|===
 ...
 |dlrm_v4_hstu |adam |global_batch_size |unconstrained |global batch size; the reference per-rank `BATCH_SIZE` multiplied by the world size |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_base_learning_rate |unconstrained |dense target (post-warmup) learning rate; the warmup ramps to this value and then holds it, there is no decay. Refer to the section "dlrm_v4_hstu reference convergence coverage" for the batch sizes at which reference convergence points exist |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_name |`Adam` |dense optimizer name; fixed |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_adam_beta_1 |0.95 |Adam beta1 |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_adam_beta_2 |0.999 |Adam beta2 |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_adam_epsilon |1e-8 |Adam epsilon |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_weight_decay |0.0 |weight decay |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_gradient_clip_norm |1.0 |gradients are clipped above this norm threshold; clipping is applied to the **dense** parameters only, the fused sparse optimizer is unaffected |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_sparse_name |`RowWiseAdagrad` |sparse (embedding table) optimizer name; fixed, fused FBGEMM TBE |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_sparse_base_learning_rate |must equal opt_base_learning_rate |sparse embedding learning rate; the reference ramps both channels in lockstep, so this tracks whichever dense learning rate the submitter chooses |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_sparse_epsilon |1e-8 |RowWiseAdagrad epsilon |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_sparse_weight_decay |0.0 |RowWiseAdagrad weight decay |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_learning_rate_warmup_steps |24000 |steps taken for the linear warmup ramp |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |opt_learning_rate_warmup_start_lr |0.0 |learning rate at global step 0; the warmup always begins from zero |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |max_sequence_length |4096 |maximum HSTU sequence length |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |hstu_attn_num_layers |fixed to reference (3) |number of HSTU attention layers; part of the model definition and may not be changed in the closed division |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |history_length |fixed to reference (4086) |user interaction history length; part of the model definition and may not be changed in the closed division |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |train_samples |2290835423 |number of training samples in one epoch; fixed by the dataset |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |eval_samples |2058204 |size of the held-out future evaluation window; every evaluation scores the entire window, subsampling is not allowed |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
 |dlrm_v4_hstu |adam |eval_every_data_pct |0.001 |evaluation cadence as a fraction of the training stream; fixed. The mutually exclusive per-window cadence (`eval_every_n_windows`) must be disabled |link:https://github.com/mlcommons/training/blob/master/recommendation/generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin[reference code] |v6.1
|===
...
Since training large language models is very expensive, the task force aims to limit hyperparameter searches. Thus the allowed range of batch sizes and corresponding learning rates are fixed as follows.

|===
 |global_batch_size |opt_base_learning_rate |opt_learning_rate_warmup_steps
...
* GBS within range but not listed: opt_base_learning_rate = 0.000024 * sqrt(GBS / 16384), rounded to the **9th** decimal place.
...
==== dlrm_v4_hstu reference convergence coverage

The target (post-warmup) learning rate and the global batch size (GBS) are the two tunable hyperparameters for this benchmark; the starting learning rate is always `0` and the warmup is always 24000 steps, so only the target learning rate varies. The reference sweep covers three global batch sizes, across which the target learning rate scales linearly:
...
These are the points at which reference convergence points exist. Submitters at other batch sizes should expect RCP interpolation; if the RCP checker cannot cover the chosen batch size, request new RCPs from the task force or normalize the score if permissible.

The closed division loss function is the masked binary cross-entropy with logits described in the link:https://github.com/mlcommons/training/blob/master/recommendation/README.MD#loss-function[reference implementation README].

Refer to the `dlrm_v4_hstu` compliance checker configuration at link:https://github.com/mlcommons/logging/blob/master/mlperf_logging/compliance_checker/training_6.1.0/closed_dlrmv4.yaml[closed_dlrmv4.yaml] and the reference convergence points at link:https://github.com/mlcommons/logging/blob/master/mlperf_logging/rcp_checker/training_6.1.0/rcps_dlrmv4.json[rcps_dlrmv4.json]. Note that the compliance checker and the submission logs use the short benchmark name `dlrmv4`.
...
|Commerce|Recommendation |dlrm_v4_hstu|Every `CEIL(0.001 * 2290835423 / global_batch_size) * global_batch_size` training samples, i.e. 2293760 samples at each of GBS 8192, 16384 and 32768. Skipping all evaluations before `CEIL(FLOOR((4480 * global_batch_size + 135331840) / 3) / eval_interval_samples) * eval_interval_samples` samples |v6.1
|===
...
|Commerce |Recommendation |10 |v6.1
|===
...
Cherry-picking or unfairly advantaged submissions cannot always be conclusively proven. If such behavior is suspected during the review period, the review chairs may take corrective action. Suspicion may arise from inconsistencies in log files, significant deviations from other results obtained on equivalent hardware/software, or any other equivalent methodology. Corrective actions may include, but are not limited to, requesting a new set of runs using the exact same software and hyperparameters as the original submission, or requiring withdrawal of any result deemed to be in violation. The above guidelines provide a general framework because each situation is unique. The review chairs have the authority to determine the appropriate course of action depending on the specific issue.
...
| dlrm_v4_hstu | Training samples starting from 0 (integer) | v6.1
|===
...
** Convergence is reported in absolute training samples, not as a fraction of an epoch: each `eval_accuracy` line must carry a `samples_count` entry in its metadata, counted from 0, and that is the value the RCP checker extracts. The benchmark converges well within the first epoch (roughly 3-5% of one pass over the 2290835423 training samples), so a run is not expected to reach `epoch_num = 1`.

** Every evaluation must score the entire held-out window of 2058204 samples. Subsampling the evaluation set is not allowed.

** The reference computes the sparse embedding all-to-all in fp32 in both directions. Submitters may lower it under the general <<Numerical formats>> rule; as with any precision change, the resulting run must still satisfy the RCP convergence test.
...
== Appendix: v6.0 Specific Rules

=== Small LLM MoE pretraining  (GPT-OSS 20B)

According to the hyperparameter rules for GPT-OSS-20B, `opt_end_learning_rate` should be `0.1 * opt_base_learning_rate`. But due to a bug in the reference implementation where the default value (`4e-5`) was set for `opt_end_learning_rate`, all the RCPs for GPT-OSS-20B use `opt_end_learning_rate = 4e-5` instead of `opt_end_learning_rate = opt_base_learning_rate * 0.1`

Since we are close to submissions and do not expect convergence to be drastically impacted by either of the `opt_end_learning_rate` values, just for v6.0 only, we will allow submissions that use either opt_end_learning_rate = 4e-5 or opt_end_learning_rate = opt_base_learning_rate * 0.1 as long as they PASS the current RCPs. 
